...
def get_model(model_type, num_classes=2, input_dim=None, max_features=None, max_length=None):
    """
    ...
    if model_type == "logistic":
        ...
        if not check_tensorflow():
            raise ImportError("TensorFlow is not available or has issues. Please install it with: pip install tensorflow")
        if input_dim is None or max_features is None or max_length is None:
            raise ValueError("For LSTM model, input_dim, max_features, and max_length must be provided")
        ...
        try:
            ...
            raise ImportError(f"Failed to create LSTM model due to TensorFlow issues: {str(e)}")
        
    elif model_type == "bilstm":
        if not check_tensorflow():
            raise ImportError("TensorFlow is not available. Please install it with: pip install tensorflow")
        if input_dim is None or max_features is None or max_length is None:
            raise ValueError("For BiLSTM model, input_dim, max_features, and max_length must be provided")
        ...
        try:
            ...
            raise ImportError(f"Failed to create BiLSTM model due to TensorFlow issues: {str(e)}")
        
    elif model_type == "cnn":
        if not check_tensorflow():
            raise ImportError("TensorFlow is not available. Please install it with: pip install tensorflow")
        if input_dim is None or max_features is None or max_length is None:
            raise ValueError("For CNN model, input_dim, max_features, and max_length must be provided")
        ...
        try:
            ...
            raise ImportError(f"Failed to create CNN model due to TensorFlow issues: {str(e)}")
        
    elif model_type == "transformer":
        if not check_tensorflow():
            raise ImportError("TensorFlow is not available. Please install it with: pip install tensorflow")
        if input_dim is None or max_features is None or max_length is None:
            raise ValueError("For Transformer model, input_dim, max_features, and max_length must be provided")
        ...
        try:
            ...
            attention = keras.layers.MultiHeadAttention(num_heads=8, key_dim=128)(embedding, embedding)
            ...
            raise ImportError(f"Failed to create Transformer model due to TensorFlow issues: {str(e)}")
...
def get_model_info(model_type):
    ...
    model_info = {
        ...
        "naive_bayes": {
            ...
            "pros": ["Fast", "Good for text classification", "Works well with small datasets"],
            "cons": ["Assumes feature independence", "Can be outperformed by other methods"]
        },
        "xgboost": {
            "name": "XGBoost",
        ...
        "lstm": {
            ...
            "pros": ["Captures sequential patterns", "Good for text", "Handles long sequences"],
    ...
    return model_info.get(model_type, {"name": "Unknown", "type": "Unknown", "description": "Unknown model type"})
...
def compare_models():
    ...
    model_types = ["logistic", "svm", "random_forest", "naive_bayes", "xgboost", 
