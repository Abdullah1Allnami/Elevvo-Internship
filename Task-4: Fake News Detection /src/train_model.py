...
class ModelTrainer:
    ...
    def load_and_preprocess_data(self, fake_path, true_path):
        ...
        if self.model_type in [
            ...
            X_train_processed, X_test_processed = self.preprocessor.vectorize_text(
                X_train, X_test
            )
        ...
        # For neural networks, use tokenization
            ...
            # Pad sequences
            max_length = 200
            X_train_processed = pad_sequences(
                X_train_seq, maxlen=max_length, padding="post"
            )
            X_test_processed = pad_sequences(
                X_test_seq, maxlen=max_length, padding="post"
            )
    ...
    def save_model_and_metadata(self, tokenizer=None, evaluation_results=None):
        ...
        if tokenizer is not None:
            tokenizer_path = os.path.join(
                self.model_dir, f"{model_filename}_tokenizer.pkl"
            )
    ...
    def hyperparameter_tuning(self, X_train, y_train):
        ...
        param_grids = {
            "logistic": {"C": [0.1, 1, 10, 100], "solver": ["liblinear", "lbfgs"]},
...
def main():
    ...
    # Initialize trainer
    trainer = ModelTrainer(model_type=args.model, save_model=not args.no_save)

    # Load and preprocess data
    X_train, X_test, y_train, y_test, tokenizer = trainer.load_and_preprocess_data(
        fake_path, true_path
    )
