...
class FakeNewsDetectionSystem:
    ...
    def train_model(self, model_type="logistic", tune_hyperparams=False, save_model=True):
        """
    ...
    def predict(self, text, model_type=None):
        ...
        if model_type is None:
            model_type = max(self.trained_models.keys(), 
                           key=lambda x: self.trained_models[x]['results']['accuracy'])
        
        if model_type not in self.trained_models:
            print(f"❌ Model {model_type} not found. Available models: {list(self.trained_models.keys())}")
            return None
...
Examples:
  ...
  python main.py --info                          # Show model information
    ...
    parser.add_argument('--predict', type=str, 
                       help='Predict if a news article is fake or true')
    ...
    parser.add_argument('--model', type=str, 
                       help='Specify model type for prediction')
    parser.add_argument('--info', type=str, nargs='?', const='all',
